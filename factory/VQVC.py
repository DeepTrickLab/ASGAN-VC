...
class Decoder(nn.Module):
    def __init__(self, in_channel, channel):
        ...
        for i in range(1, 4, 1):
            block = GBlock(
                in_channel // 2 ** (i), in_channel // 2 ** (i), channel, num_groups
            )
            blocks.append(block)
        for i in range(1, 4, 1):
            block = GBlock(
    ...
    def forward(self, q_after, sp_embed, std_embed):
        ...
        for i, (block, block_refine, res, scale_factor) in enumerate(
            zip(self.blocks, self.blocks_refine, self.resblock, self.z_scale_factors)
        ):
...
class VQVC(nn.Module):
    def __init__(
        self, in_channel=80, channel=512, n_embed=128,
    ):
        super().__init__()
        ...
        for i in range(3):
            blocks += [
                nn.Sequential(
                    *[
                        nn.Conv1d(
                            in_channel // 2 ** (i), channel, 4, stride=2, padding=1
                        ),
                        nn.LeakyReLU(),
        ...
        self.dec = Decoder(in_channel, channel)
